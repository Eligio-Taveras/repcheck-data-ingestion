...
class DoobieBillHistoryArchiverUnitSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  "archiveBill" should "execute the full archive flow against H2 and return a history id" in {
    val historyId = archiver.archiveBill("118-HR-300").transact(h2xa).unsafeRunSync()
    historyId should be > 0L
  }
