...
class AmendmentTextExtractorSpec extends AnyFlatSpec with Matchers {
  ...
  "extractStream" should "dispatch HTML to CrecHtmlExtractor" in {
    ...
    val text  = AmendmentTextExtractor.extractStream[IO](bytes, "HTML").compile.toList.unsafeRunSync().mkString(" ")
    text should include("plain amendment text")
  }

  it should "raise AmendmentTextProcessingFailed for unknown formatType" in {
    val bytes = Stream.empty.covary[IO]
    val attempt = AmendmentTextExtractor
      .extractStream[IO](bytes, "Formatted XML")
      .compile
      .toList
      .attempt
      .unsafeRunSync()
    attempt match {
      case Left(_: AmendmentTextProcessingFailed) => succeed
