 ...
 * a non-2xx response. Carries the HTTP status code (via the [[HttpStatusError]] trait) so [[MembersApiErrorClassifier]]
 * can decide whether the failure is transient (retry) or systemic (fail fast).
 *
