...
import com.repcheck.bills.text.embedding.{EmbeddingConfig, NoOpEmbeddingService, OllamaEmbeddingService}
...
class MetadataToVectorSearchLifecycleSpec
  ...
  private val memberLookupRepo = new DoobieMemberLookupRepository()
  ...
  private def buildChecker(): BillTextAvailabilityChecker[IO] = {
    ...
    val eventPublisher =
      new DefaultIngestionEventPublisher[IO](pubsubPublisher, topicName.toString, "lifecycle-test")
