...
/**
 * Streaming dispatcher for bill-text extraction. Consumes a `Stream[F, Byte]` (typically the HTTP response body
 * straight from [[repcheck.ingestion.bills.text.download.BillTextDownloader.streamBody]]) and emits the extracted prose
 * as a `Stream[F, String]` — one fragment per natural unit of the source format (one PDF page, one HTML `characters`
 * event, one XML `CHARACTERS` event, one fs2 byte-decoded chunk for plain text).
 *
...
object BillTextExtractor {
  ...
  /**
   * Extract bill text from an upstream byte stream as a stream of normalized text fragments. Format strings come from
   * Congress.gov (`bill.textFormat` field) so this matches their case-sensitive labels exactly.
   *
   * @param bytes
   *   the byte stream to extract from. Typically the HTTP response body from
   *   [[repcheck.ingestion.bills.text.download.BillTextDownloader.streamBody]]. Backpressure flows up through this
   *   stream — bytes are pulled at the rate the embedding stage downstream can consume.
   * @param textFormat
  ...
  def extractStream[F[_]: Async](bytes: Stream[F, Byte], textFormat: String): Stream[F, String] =
    textFormat match {
      case "Formatted Text" => HtmlStreamExtractor.extract[F](bytes)
