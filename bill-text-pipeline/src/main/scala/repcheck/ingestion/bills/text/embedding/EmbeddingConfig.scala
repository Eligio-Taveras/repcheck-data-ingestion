 ...
 * pipeline tries to hit on every call. Sweet spot on RTX 2070 SUPER + qwen3-embedding:0.6b is batch=50 — the smaller
 * model frees ~5 GB of VRAM vs the 4B baseline, lifting GPU-saturation batch size from ~10 to ~50. Override lower if
 * running against a larger model. Tunable via `OLLAMA_EMBED_BATCH_SIZE`. Must be `> 0`.
 *
