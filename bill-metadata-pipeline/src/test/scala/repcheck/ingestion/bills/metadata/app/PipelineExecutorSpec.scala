...
class PipelineExecutorSpec extends AnyFlatSpec with Matchers {
  ...
  it should "log the pipeline summary with correct counts" in {
    ...
    val logMessage = logger.messages
      .find(_.contains("Pipeline completed:"))
      .getOrElse(fail("expected a 'Pipeline completed' summary log"))
  ...
  it should "return ExitCode.Error when all results fail" in {
    ...
    val stream = Stream.emits(
      List(
        ProcessingResult.Failed("bill-1", "error 1"),
        ProcessingResult.Failed("bill-2", "error 2"),
      )
    )
  ...
  it should "return ExitCode.Success for mixed succeeded and skipped results with zero failures" in {
    ...
    val stream = Stream.emits(
      List(
        ProcessingResult.Succeeded("bill-1"),
        ProcessingResult.Skipped("bill-2", "unchanged"),
        ProcessingResult.Succeeded("bill-3"),
      )
    )
