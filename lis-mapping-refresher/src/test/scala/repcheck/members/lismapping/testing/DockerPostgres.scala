...
/**
 * Docker-backed AlloyDB Omni provisioner for lis-mapping-refresher integration tests. Mirrors the structure used by
 * bills-common's fixture but is self-contained since lis-mapping-refresher depends only on members-common.
 */
object DockerPostgres {

  private val dbName: String          = "repcheck_lis_test"
  ...
  private def acquire: IO[ContainerHandle] = IO.blocking {
    val containerName = s"repcheck-lis-test-${java.util.UUID.randomUUID().toString.take(8)}"
