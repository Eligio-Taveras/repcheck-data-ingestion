...
/**
 * Classifier for [[repcheck.ingestion.bills.textcheck.api.BillTextApiClient]] HTTP failures. Treats the standard
 ...
 * project-declared [[BillTextApiHttpError]] instead of the ingestion-common `CongressGovApiException` so that the
 * sbt-exception-uniqueness plugin's project-exceptions-only check sees a locally-declared Throwable.
 */
object BillTextApiErrorClassifier extends ErrorClassifier {

  private val transientStatusCodes: Set[Int] = Set(429, 500, 502, 503, 504)

  def classify(error: Throwable): ErrorClass =
    error match {
      case e: BillTextApiHttpError if transientStatusCodes.contains(e.statusCode) =>
        ErrorClass.Transient
      case _: BillTextApiHttpError =>
        ErrorClass.Systemic
      case _ =>
        ErrorClass.Systemic
    }
