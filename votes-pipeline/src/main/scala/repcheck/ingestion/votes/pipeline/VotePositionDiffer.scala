...
/**
 * Provides a `Differ[List[VotePositionDO]]` instance that treats the position list as a **set keyed by `memberId`**.
 * Called by [[VoteChangeDetector]] to decide whether an incoming vote's positions differ from the stored set.
 *
 ...
 * `.pairBy(_.memberId)` lifts the list differ from the default index-based comparison to member-keyed matching — so
...
object VotePositionDiffer {
   ...
   * so this compares every field — including `position`, `partyAtVote`, and `stateAtVote`. `.toString` is used only as
   * the printable rendering inside `ValueResult`.
   */
  given votePositionDiffer: Differ[VotePositionDO] =
    Differ.useEquals[VotePositionDO](_.toString)

  /**
   * List differ keyed by `memberId` — the canonical identity for a position within a single vote, per §6.4. Built from
   * [[votePositionDiffer]] via difflicious's `seqDiffer` implicit, then reconfigured with `.pairBy(_.memberId)` to get
   * set-style matching. `Differ.seqDiffer` is invoked directly rather than via `summon` to avoid a self-referential
   * resolution against this very `given`.
   */
  given votePositionsDiffer: Differ[List[VotePositionDO]] =
    Differ.seqDiffer[List, VotePositionDO].pairBy(_.memberId)
