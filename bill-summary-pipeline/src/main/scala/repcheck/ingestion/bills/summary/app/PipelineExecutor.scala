 ...
 * time.
 *
 * Mirrors the votes-pipeline `PipelineExecutor` deliberately — when this pattern stabilizes across enough pipelines we
 * can promote a shared executor to ingestion-common, but for now keeping a copy per pipeline avoids forcing every
 * pipeline to evolve in lockstep.
 */
private[app] object PipelineExecutor {
   ...
   *   independently by the processor for per-bill log search.
   * @param stepRunId
   ...
   * individual per-bill log entries. Each entry carries the entityId (e.g. bill natural key), the error classifier, and
