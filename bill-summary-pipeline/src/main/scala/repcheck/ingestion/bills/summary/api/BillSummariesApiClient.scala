 ...
 * key in the processor. `fetchAll` (inherited from [[CongressGovPaginatedClient]]) drains every page across the
 * configured `fromDateTime`/`toDateTime` window with the standard short-page retry semantics shared with
