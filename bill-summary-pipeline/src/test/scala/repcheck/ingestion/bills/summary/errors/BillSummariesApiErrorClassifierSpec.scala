...
class BillSummariesApiErrorClassifierSpec extends AnyFlatSpec with Matchers {
  ...
  it should "classify 5xx as Transient" in {
    val _ = BillSummariesApiErrorClassifier.classify(BillSummariesApiHttpError(500, "server")) shouldBe ErrorClass.Transient
    val _ = BillSummariesApiErrorClassifier.classify(BillSummariesApiHttpError(502, "bad gw")) shouldBe ErrorClass.Transient
    val _ = BillSummariesApiErrorClassifier.classify(BillSummariesApiHttpError(503, "down")) shouldBe ErrorClass.Transient
  ...
  it should "classify 4xx (other than 429) as Systemic" in {
    val _ = BillSummariesApiErrorClassifier.classify(BillSummariesApiHttpError(400, "bad request")) shouldBe ErrorClass.Systemic
    val _ = BillSummariesApiErrorClassifier.classify(BillSummariesApiHttpError(401, "unauthorized")) shouldBe ErrorClass.Systemic
    val _ = BillSummariesApiErrorClassifier.classify(BillSummariesApiHttpError(403, "forbidden")) shouldBe ErrorClass.Systemic
