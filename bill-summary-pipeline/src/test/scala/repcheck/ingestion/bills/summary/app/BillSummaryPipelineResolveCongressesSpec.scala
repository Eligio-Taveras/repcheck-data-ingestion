...
import repcheck.ingestion.bills.summary.app.BillSummaryPipeline.AppConfig
 ...
 *
 * Mirrors [[repcheck.ingestion.members.profile.app.MemberProfilePipelineResolveCongressesSpec]] — same precedence
 * policy, same fall-back semantics — so the two specs intentionally read identically.
 */
class BillSummaryPipelineResolveCongressesSpec extends AnyFlatSpec with Matchers {
  ...
  private val baseConfig = AppConfig(
    ...
    pipeline = BillSummaryConfig(
      ...
      congresses = Nil,
      stepName = "bill-summary-pipeline",
      httpConcurrency = 1,
    ),
  )
  ...
  private def freshXa(name: String): Transactor[IO] =
    Transactor.fromDriverManager[IO](
      ...
      url = s"jdbc:h2:mem:billSummaryResolveCongresses_$name;DB_CLOSE_DELAY=-1",
      user = "",
      password = "",
      logHandler = None,
    )
  ...
  "resolveCongresses" should "use the env var when BILL_SUMMARY_CONGRESSES is set, ignoring config and DB" in {
    ...
    val envGetter: String => Option[String] = {
      case "BILL_SUMMARY_CONGRESSES" => Some("117,118,119")
      case _                         => None
    }

    val result = BillSummaryPipeline.resolveCongresses[IO](config, xa, logger, envGetter).unsafeRunSync()
    result shouldBe List(117, 118, 119)
  }

  it should "trim whitespace and skip empty tokens in BILL_SUMMARY_CONGRESSES" in {
    ...
    val result = BillSummaryPipeline.resolveCongresses[IO](baseConfig, xa, logger, envGetter).unsafeRunSync()
    result shouldBe List(117, 118, 119)
  }

  it should "treat an empty BILL_SUMMARY_CONGRESSES env var as if unset (falls through to next layer)" in {
    val xa     = freshXa("env_empty")
    val logger = new StubLogger
    val config = baseConfig.copy(pipeline = baseConfig.pipeline.copy(congresses = List(118, 119)))
    val envGetter: String => Option[String] = {
      case "BILL_SUMMARY_CONGRESSES" => Some("   ")
      case _                         => None
    }
  ...
  it should "use config.pipeline.congresses when env is unset and config is non-empty" in {
    ...
    val result = BillSummaryPipeline.resolveCongresses[IO](config, xa, logger, _ => None).unsafeRunSync()
    result shouldBe List(118, 119)
  }
  ...
  it should "fall back to the bills table when both env and config are empty" in {
    ...
    val result = BillSummaryPipeline.resolveCongresses[IO](baseConfig, xa, logger, _ => None).unsafeRunSync()
