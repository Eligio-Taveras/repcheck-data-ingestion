...
| 0.2 | `repcheck-shared-models` | repcheck-shared-models repo | Field additions: `AmendmentDO` (+4 fields after 2026-05-05 live-API revision): `parentAmendmentId`, `proposedDate` (restored — confirmed real via live API), `latestActionTime`, `lastTextCheckAt`. **`billId` now always populated with resolved ancestor bill** (no separate `effectiveBillId`). **`chamber` tightened to `chamber_type NOT NULL`** (per L9). New `LegislationRef` sealed type. New `AmendedAmendmentDTO` for sub-amendment recursion (verified live). New `ResourceLinkDTO` for `{count, url}` link sub-objects (used by `actions`, `textVersions`, `cosponsors`, `amendmentsToAmendment` fields on `AmendmentDetailDTO`). New text DTOs: `AmendmentTextItemDTO`, `AmendmentFormatDTO`, `AmendmentTextResponseDTO`. Custom Decoders for the `type` → `amendmentType`/`billType` field rename (mirrors `BillReferenceDTO.decoder` precedent). `AmendedBillDTO.number` corrected to `Option[String]` (API returns string). New `AmendmentDetailDTO.toDO(billId, sponsorMemberId, parentAmendmentId)` overload. **No analysis types** — deferred. | [P7.0](PRODUCTION_TASKS.md#p70) |
